...
{
    if (USBD_SetupPacket.wIndexL == usbd_hid_if_num || USBD_SetupPacket.wIndexL == usbd_webusb_if_num) {         /* IF number correct? */
...
{
    if (USBD_SetupPacket.wIndexL == usbd_hid_if_num ||
        USBD_SetupPacket.wIndexL == usbd_webusb_if_num) {   /* IF number correct? */
        switch (USBD_SetupPacket.bRequest) {
            case HID_REQUEST_SET_REPORT:
