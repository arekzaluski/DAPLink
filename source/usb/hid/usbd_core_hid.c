...
__weak BOOL USBD_ReqGetDescriptor_HID(U8 **pD, U32 *len)
{
    switch (USBD_SetupPacket.wValueH) {
        case HID_HID_DESCRIPTOR_TYPE:
            if (USBD_SetupPacket.wIndexL != usbd_hid_if_num && USBD_SetupPacket.wIndexL != usbd_hid_webusb_if_num) {
...
{
    if (USBD_SetupPacket.wIndexL == usbd_hid_if_num || USBD_SetupPacket.wIndexL == usbd_hid_webusb_if_num) {         /* IF number correct? */
